...
export class AccountMonitor {
  ...
  getFormattedStatus(status: TradingStatus): string {
    ...
    const border = (s: string) => c(ansi.purple, s);
    const header = (s: string) => c(ansi.bold + ansi.yellow, s);
    const label = (s: string) => c(ansi.cyan, s);
    const subtle = (s: string) => c(ansi.gray, s);

    const lines = [
      '',
      border('╔══════════════════════════════════════════════════════════╗'),
      border(`║${header('     Polymarket Account Monitor - Open Positions          ')}║`),
      border('╚══════════════════════════════════════════════════════════╝'),
      `${label('👤 Account:')} ${status.user.substring(0, 10)}...${status.user.substring(status.user.length - 8)}`,
      `${label('🕐 Last Updated:')} ${subtle(new Date(status.lastUpdated).toLocaleString())}`,
    ];
    ...
    if (status.openPositions.length > 0) {
      ...
      lines.push('');
      lines.push(
        `${label('💼 Currently Open Positions:')} ${subtle(`(showing ${displayCount} of ${status.openPositions.length})`)}`
      );
      status.openPositions.slice(0, 10).forEach((position, index) => {
        ...
        lines.push(
          `   ${subtle(`${index + 1}.`)} ${outcome}: ${quantity} shares @ $${price}`
        );
        ...
        if (value === 0 && position.initialValue) {
          ...
          lines.push(`${subtle('      Current Value:')} $${valueStr} ${subtle(`(Initial: $${initialValueStr})`)}`);
        } else {
          lines.push(`${subtle('      Current Value:')} $${valueStr}`);
        }
        
        lines.push(`${subtle('      Market:')} ${shortTitle}`);
      });
    } else {
      lines.push('');
      lines.push(`${label('💼 Currently Open Positions:')} ${subtle('No active positions')}`);
    }
